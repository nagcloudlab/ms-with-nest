...
export class AppController {
  ...
  @Get(":id")
  getProduct(@Param("id") productId: number): ProductAggregate {
  ...
  createProduct(@Body() body) {
    ...
    const stream1 = this.appService.createProduct(product)
    // const reviews = body.reviews;
    // reviews.forEach(review => {
    //   const stream2 = this.appService.createReview(product.id, review)
    // })
    return zip(stream1)
      .pipe(map(responses => [responses[0].data]))
  }
